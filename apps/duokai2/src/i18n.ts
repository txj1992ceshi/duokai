...
export const dictionaries: Record<LocaleCode, Dictionary> = {
  'zh-CN': {
    ...
    dashboard: {
      ...
      recentLogs: '最近日志',
      noLogs: '暂无日志。',
      installChromium: '固定单引擎；完整性校验未通过时阻止启动',
    },
    profiles: {
      title: '环境工作台',
      manageProfiles: '环境',
    ...
    settings: {
      ...
      appData: '应用数据目录',
      profiles: '环境目录',
      chromiumBinary: 'CloakBrowser 固定版本与缓存',
      missingChromium: 'CloakBrowser 固定版本信息不可用；启动已阻止。',
      language: '界面语言',
  ...
  'en-US': {
    ...
    dashboard: {
      ...
      noLogs: 'No logs yet.',
      installChromium: 'Pinned single engine; launch is blocked until integrity checks pass',
    },
    profiles: {
    ...
    settings: {
      ...
      appData: 'App data',
      profiles: 'Profiles',
      chromiumBinary: 'CloakBrowser pinned version and cache',
      missingChromium: 'Pinned CloakBrowser metadata is unavailable; launch is blocked.',
