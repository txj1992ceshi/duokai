...
5. Control plane creates a control task.
6. The target desktop agent claims the task.
7. Local desktop runtime validates lease, policy, and workspace isolation.
8. Local desktop runtime executes the action.
9. Agent reports compact task events and audit data back to control plane.
10. Large artifacts are uploaded as file-backed objects, with only metadata stored in Mongo.
